...
DocumentDialog.prototype.initFunctions = function() {
	...
	this.showPagination = function() {
		$(paginationTools).show().removeClass('hidden');
	};
	this.hidePagination = function() {
		$(paginationTools).hide().addClass('hidden');
	};
	...
	if (EditionProperties.pdfLink) {
		...
		var pdf = $('<a class="button tools-pdf">PDF</a>').appendTo(pdfLi);
	...
	if (EditionProperties.dfgViewer) {
		...
		var dfgViewer = $('<a class="button tools-dfg">DFG</a>').appendTo(dfgLi);
	...
	if (EditionProperties.connectable) {
		...
		var link = $('<a class="button icon-unlocked"></a>').appendTo(linkLi);
...
DocumentDialog.prototype.resize = function() {
	...
	var containerDivHeight = (height - paddingY - this.docContainerDiv.offset().top + this.container.offset().top - this.docTypeDiv.height());

	this.docContainerDiv.css('height', containerDivHeight + "px");
	this.docContainerDiv.css('width', (width - paddingX) + "px");

	var docContainerDivMaxHeight = (height - paddingY - this.docContainerDiv.offset().top + this.container.offset().top);

	this.docContainerDiv.css('maxHeight', docContainerDivMaxHeight + "px");
...
DocumentDialog.prototype.setDocType = function(type, id) {
	...
	// Set classes before the actual rendering so relevant elements will be positioned correctly
