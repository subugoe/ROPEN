...
var EditionGui = (function() {
	 ...
	 */
	gui.initializeFolders = function(number) {
		"use strict";
	...
	gui.checkBounds = function() {
		...
		if ($(gui.containerDiv).height() !== highest + EditionProperties.margin) {
			$(gui.containerDiv).css('height', (EditionProperties.margin + highest) + 'px');
		}
		if ($(gui.containerDiv).width() !== farthest + EditionProperties.margin) {
			$(gui.containerDiv).css('width', (EditionProperties.margin + farthest) + 'px');
		}
	};
	 ...
	 */
	gui.createDialog = function(headline, content, event, sx, sy, onclose) {
		"use strict";
		var gui = this;
	 ...
	 */
	gui.removeFolder = function(folder) {
		"use strict";
		folder.remove();
		for (var i = 0; i < this.folders.length; i++) {
	 ...
	 */
	gui.openDocument = function(evt, document, page, type, position, entity, folderNumber) {
		"use strict";
		var gui = this;
		var candidates = [];
		var doc = {
		...
		// open default view
		if (folderNumber >= 0) {
			gui.openDefaultView(candidates[folderNumber], doc);
		} else {
	 ...
	 * @param doc
	 */
	gui.openDefaultView = function(folder, doc) {
		"use strict";
	 ...
	 * @param linkList
	 */
	gui.generateMagneticLink = function(gui, linkList) {
		"use strict";
	 ...
	 * @param linkList
	 */
	gui.updateLinks = function(gui, linkList) {
		"use strict";
		$(linkList).empty();
	 ...
	 * @param controls
	 */
	gui.addMagneticLink = function(gui, controls) {
		"use strict";
		var linkList = $("<div/>");
		...
		magneticLink.click(function(evt) {
			...
			generateButton.click(function() {
				gui.generateMagneticLink(gui, linkList);
			});
			$(linkList).appendTo(p);
	...
	gui.addControls = function() {
		...
		if (EditionProperties.magneticLink) {
			gui.addMagneticLink(gui, controls);
		}
